...
def generate_taunt(human_pos, ai_pos):
    return call_ollama(f"你是一个毒舌五子棋AI。人类下在{human_pos}，我极其轻蔑地下在{ai_pos}。请用15字以内嘲讽：",
                       fallback="破绽百出！")


def generate_checkmate_taunt(is_human_threat):
    if is_human_threat:
        return call_ollama("人类刚走出必赢活四，请用15字以内找个极其荒谬的借口死不认输。",
                           fallback="肯定是风把棋子吹跑了！")
    else:
        return call_ollama("你刚走出必赢活四。请用15字以内发表极其嚣张的宣告。", fallback="放弃挣扎吧！")


def generate_endgame_taunt(is_human_win):
    if is_human_win:
        return call_ollama("你彻底输给了人类。用15字找破防借口。", fallback="我的CPU刚才打喷嚏了！")
    else:
        return call_ollama("你连成5子碾压了人类。发表终极胜利宣言。", fallback="蝼蚁也敢与皓月争辉？")
...
while True:
    ...
    if contours:
        ...
        if len(approx) == 4:
            ...
            M = cv2.getPerspectiveTransform(rect, np.array([[0, 0], [599, 0], [599, 599], [0, 599]], dtype="float32"))
                    ...
                    if winner != 0:
                        ...
                        if not fatal_warned:
                            ...
                            if threat != 0:
                                ...
                                print(f"\n🚨 活四预警！\n🤖 AI: {taunt}")
                                speak_taunt(taunt)
                                cv2.putText(warped, "CHECKMATE!", (150, 250), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 255),
                                            3)
                        ...
                        if target_black == target_white:
                            # 数量相等，人类刚帮AI下完了白棋，或者刚开局
                            if ai_planned_move is not None:
                                print(f"\n[系统] ✅ 确认白棋落子！红圈清除。")
                                ai_planned_move = None  # 核心修复：立刻清除红圈！

                        elif target_black == target_white + 1:
                            # 黑比白多一个，绝对是人类刚下完黑棋，该AI算棋了！
            ...
            if ai_planned_move is not None:
                ...
                cv2.putText(warped, "PLACE WHITE", (ai_x + 15, ai_y - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255),
                            2)

            cv2.rectangle(warped, (0, 550), (600, 600), (30, 30, 30), -1)

            # 动态回合指示器
            if target_black == target_white:
                cv2.putText(warped, "YOUR TURN: Place Black", (15, 575), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            elif target_black == target_white + 1:
                cv2.putText(warped, "AI TURN: Place White", (15, 575), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
            else:
                cv2.putText(warped, "ERROR: Pieces Mismatch!", (15, 575), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

            if stable_frames < 15:
                cv2.putText(warped, f"Wait... {stable_frames}/15", (420, 575), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                            (0, 165, 255), 2)
            else:
                cv2.putText(warped, "READY", (480, 575), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
